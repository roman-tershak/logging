...
public class ExtLoggerFilterTest {

    @Test
    public void testExtLoggerEventsArePassedThrough() {
        ExtLoggerFilter unit = getUnit();

        assertThat(unit.filter(createExtLogEvent(TRACE, FAIL, "Message {}")), equalTo(NEUTRAL));
        assertThat(unit.filter(createExtLogEvent(DEBUG, null, "Message {}")), equalTo(NEUTRAL));
        assertThat(unit.filter(createExtLogEvent(INFO, null, "Message {}")), equalTo(NEUTRAL));
        assertThat(unit.filter(createExtLogEvent(WARN, SUCCESS, "Message {}", "warning")), equalTo(NEUTRAL));
        assertThat(unit.filter(createExtLogEvent(ERROR, null, "Message {}", "Failure")), equalTo(NEUTRAL));
        assertThat(unit.filter(createExtLogEvent(FATAL, FAIL, "Message {}")), equalTo(NEUTRAL));
    }

    @Test
    public void testRegularLoggerEventsAreFilteredOut() {
        ExtLoggerFilter unit = getUnit();
    ...
    @Test
    public void testRegularLoggerEventsArePassedThroughIfOnMismatchIsAccept() {
        ExtLoggerFilter unit = getUnit(DENY, ACCEPT);

        assertThat(unit.filter(createLogEvent(TRACE, FAIL, "Message {}")), equalTo(ACCEPT));
        assertThat(unit.filter(createLogEvent(DEBUG, null, "Message {}")), equalTo(ACCEPT));
        assertThat(unit.filter(createLogEvent(INFO, null, "Message {}")), equalTo(ACCEPT));
        assertThat(unit.filter(createLogEvent(WARN, SUCCESS, "Message {}", "warning")), equalTo(ACCEPT));
        assertThat(unit.filter(createLogEvent(ERROR, null, "Message {}", "Failure")), equalTo(ACCEPT));
        assertThat(unit.filter(createLogEvent(FATAL, FAIL, "Message {}")), equalTo(ACCEPT));
    }

    @Test
    public void testExtLoggerEventsAreFilteredOutIfOnMatchIsDeny() {
        ExtLoggerFilter unit = getUnit(DENY, ACCEPT);
    ...
    private ExtLoggerFilter getUnit() {
        return getUnit(NEUTRAL, DENY);
    }

    private ExtLoggerFilter getUnit(Result onMatch, Result onMismatch) {
        ExtLoggerFilter filter = ExtLoggerFilter.newExtLoggerFilterBuilder()
                .setOnMatch(onMatch)
